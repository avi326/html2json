...
- For consistent, well-structured tables, the rule-based method is recommended due to its speed, simplicity, low cost, and high success rate (99.97%).
- For complex or non-standard table structures, the LLM method is preferred for its flexibility and accuracy, despite slower performance and higher costs.
- Consider the trade-offs between speed, accuracy, and cost when choosing between the two methods.
- The detailed evaluation process, checking element-wise matching in all table sections, ensures high reliability of results for both methods.

## 8. Future Work

- Develop a hybrid approach combining rule-based and LLM methods for optimized speed and adaptability.
- Explore fine-tuning of other language models (e.g., GPT-based) to improve parsing performance.
- Investigate hybrid CPU/GPU models to balance cost and performance.
- Implement enhanced error handling and validation for the LLM method to reduce JSON syntax errors.
- Conduct more extensive testing on diverse table structures to further validate both approaches.
- Enhance the evaluation process to provide more granular insights into parsing accuracy across different table components.

## 9. Acknowledgments

This project utilized the Microsoft Phi-3 model and the Hugging Face platform for model hosting and distribution. We thank the open-source community for their contributions to the tools and libraries used in this project.
